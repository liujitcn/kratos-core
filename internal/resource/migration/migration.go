...
type Migration struct {
	registry  *gormmigration.Registry
	names     []gormmigration.ModuleName
...
func NewMigration(ctx *bootstrap.Context, databases map[string]*databaseGorm.Client, migrations module.Migrations) (*Migration, error) {
	...
	for _, value := range migrations {
		dependencies := make([]gormmigration.ModuleName, 0, len(value.Dependencies))
		for _, dependency := range value.Dependencies {
			dependencies = append(dependencies, gormmigration.ModuleName(dependency))
		}
		name := gormmigration.ModuleName(value.Name)
		contributors = append(contributors, migrationContributor{
			name:       name,
			migrations: []gormmigration.Migration{{FS: value.FS, Path: value.Path, Dependencies: dependencies}},
		})
	...
	var err error
	registry.registry, err = gormmigration.NewRegistry(contributors)
	if err != nil {
...
func (r *Migration) Run(ctx context.Context) error {
	...
	clients := r.databases
	var runner *gormmigration.Runner
	var err error
	runner, err = gormmigration.NewRunner(r.registry)
	if err != nil {
...
type migrationContributor struct {
	name       gormmigration.ModuleName
	migrations []gormmigration.Migration
}

// Name 返回迁移模块名称。
func (c migrationContributor) Name() gormmigration.ModuleName {
	return c.name
}

// Migrations 返回迁移资源快照。
func (c migrationContributor) Migrations() []gormmigration.Migration {
	return append([]gormmigration.Migration(nil), c.migrations...)
}
