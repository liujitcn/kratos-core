...
- `pkg` 提供 `biz`、`config`、`const`、`errorsx`、`module` 等公開套件。
- `api/gen/go` 提供跨專案共用的 protobuf 類型。

其他實作全部位於 `internal`，不屬於跨專案 API。快取、佇列、預設資料庫、OSS 和翻譯器由 Core 建立；快取、佇列、OSS 和翻譯器會注入 `biz.BaseCase`，同時繼續寫入 `kratos-kit/sdk.Runtime`，業務程式碼可按需從 BaseCase 或 SDK 取得。
...
`module.Resources` 是業務建立前的靜態貢獻，包含模型、遷移、OpenAPI、專案文件和 I18n；模組透過 `RegisterQueue`、`RegisterCron` 和 `RegisterSSE` 直接註冊執行期處理器。`RegisterCron` 內呼叫 `server.RegisterTask` 註冊資料庫任務執行器，`RegisterSSE` 內呼叫 `server.RegisterStream` 註冊業務流解析器；HTTP、gRPC、MCP、佇列、定時任務和 SSE 服務均由 Core 統一建立和管理。

`pkg/module` 描述宿主向 Core 提供的資源和協議註冊契約；`biz.ProviderSet` 只建立 `BaseCase`。
...
| 欄位 | 用途 |
| --- | --- |
| `OpenAPI` | OpenAPI YAML/JSON 檔案系統集合。 |
| `Docs` | 專案文件產生結果檔案系統集合，通常包含 `docs.json`。 |
| `I18n` | 宿主語言 JSON 檔案系統集合。 |

資料庫遷移透過 `module.Resources.Migrations` 提供，每個 `module.Migration` 明確宣告模組名稱、檔案系統、版本目錄根路徑和依賴模組。

宿主透過 `module.Docs`、`module.I18n` 和 `module.OpenAPI` 提供多個 `fs.FS`。Core 讀取後轉換為內部註冊表和 `pkg/biz` 查詢所需的資料結構；國際化目錄實作位於 `internal/i18n`，語言狀態和請求上下文實作位於 `internal/locale`。
...
pkg/
  ...
  const/                 公共常數
  errorsx/               統一錯誤建構
  module/                宿主向 Core 提供的資源和協議註冊契約

internal/
  application/           應用組裝、模組資源收集和統一生命週期
  runtime/               cache、queue、database、auth 和 OSS 初始化
  data/                  Core 最小模型和儲存庫
  server/                HTTP、gRPC、MCP、SSE 服務組裝
  task/, job/             任務和 Cron 內部實作
