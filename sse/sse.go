...
func NewSSE(authenticator engine.Authenticator, userToken *data.UserToken, registry *Registry, server *Server) (*SSE, func()) {
...
func (r *SSE) authenticate(request *http.Request) (*data.UserTokenPayload, error) {
	if r.authenticator == nil || r.userToken == nil {
		return nil, errorsx.Unauthenticated("SSE认证未配置")
	}
	token := request.Header.Get(engine.HeaderAuthorize)
	if token == "" {
		return nil, errorsx.Unauthenticated("SSE访问令牌为空")
	}
	if len(token) >= len(engine.BearerWord)+1 && strings.EqualFold(token[:len(engine.BearerWord)+1], engine.BearerWord+" ") {
		token = token[len(engine.BearerWord)+1:]
	}
	...
	userToken := &data.UserTokenPayload{}
